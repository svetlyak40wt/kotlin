    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        return object: KtVisitorVoid() {
            ...
            override fun visitDeclaration(declaration: KtDeclaration) {
                ...
                when (declaration) {
                    is KtProperty -> {
                        if (declaration.isVar && declaration.isLocal && canBeVal(declaration, declaration.hasInitializer())) {
                            reportCanBeVal(declaration)
                        }
                    }

                    is KtDestructuringDeclaration -> {
                        if (declaration.valOrVarKeyword?.node?.elementType == KtTokens.VAR_KEYWORD && declaration.entries.all { canBeVal(it, true) }) {
                            reportCanBeVal(declaration)
                        }
                    }
                }
            ...
            private fun canBeVal(declaration: KtVariableDeclaration, hasInitializer: Boolean): Boolean {
                val search = ReferencesSearch.search(declaration, declaration.useScope)
                if (hasInitializer) {
                    val hasWriteUsages = search.any {
                    ...
                    if (search.none()) {
                        // no initializer and no usages: do not report - probably incomplete code
                        return false
                    }
