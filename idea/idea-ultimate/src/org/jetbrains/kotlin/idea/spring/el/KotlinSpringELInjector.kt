...
class KotlinSpringELInjector : MultiHostInjector {
    companion object {
        ...
        private val contextRecorder by lazy {
            try {
                ...
                val key = Class
                        .forName("com.intellij.psi.impl.source.jsp.el.ELContextProvider")
                        .getField("ourContextProviderKey")
                        .get(null) as Key<Any>
                val contextConstructor = Class
                        .forName("com.intellij.spring.el.contextProviders.SpringElContextProvider")
                        .getConstructor(PsiElement::class.java)
                return@lazy { host: PsiElement -> host.putUserData(key, contextConstructor.newInstance(host)) }
            }
            catch(e: Exception) {
                if (e is ClassNotFoundException || e is NoSuchMethodException || e is NoSuchFieldException) return@lazy null
                throw e
            }
        }
    }
    ...
    override fun getLanguagesToInject(registrar: MultiHostRegistrar, host: PsiElement) {
        if (host is KtStringTemplateExpression) {
            ...
            if (argumentName != null && argumentName.asName.asString() != "value") return
