...
private fun createSettable(
    ...
    try {
        ...
        return currentBeanClass.getOrigin()!!.addDeclaration(prototype).apply { reformat(this) }
    }
...
private fun getSuggestedNames(
    ...
    val validator = NewDeclarationNameValidator(declaration.parent, null, NewDeclarationNameValidator.Target.VARIABLES)
        ...
        val nameExpression = object : Expression() {
            ...
            override fun calculateLookupItems(context: ExpressionContext): Array<LookupElement>? {
                ...
                return suggestedNames
                        .mapTo(LinkedHashSet()) { LookupElementBuilder.create(it) }
                        .toTypedArray()
...
private fun SpringGenerateTemplatesHolder.addCreateFunctionTemplate(
        function: KtFunction,
...
private fun SpringGenerateTemplatesHolder.addCreatePropertyTemplate(property: KtProperty, candidateBeanClasses: Array<out PsiClass>) {
...
private fun getOrCreateSetter(
        ...
        currentBeanClass: KtLightClass,
        candidateBeanClasses: Array<out PsiClass>,
        templatesHolder: SpringGenerateTemplatesHolder,
        injectionKind: SpringDependencyInjectionKind
): PsiNamedElement? {
...
private fun createSetterDependency(
    ...
    val templatesHolder = SpringGenerateTemplatesHolder(currentBean.manager.project)
...
private fun addConstructorParameter(
    ...
    val resolutionFacade = currentBeanClass.getOrigin()!!.getResolutionFacade()
    ...
    constructor.getValueParameterList()!!.addParameter(parameter)
}
...
private fun findProperConstructorAndAddParameter(
        ...
        currentBeanClass: KtLightClass,
        candidateBeanClass: PsiClass,
        holder: SpringGenerateTemplatesHolder
): PsiMethod? {
    val lightConstructor = currentBean.resolvedConstructorArgs.resolvedMethod as? KtLightMethod
    val properConstructor = lightConstructor?.getOrigin() as? KtConstructor<*> ?: return null
...
private fun createConstructorWithTemplate(
    ...
    val ktBeanClass = beanClass.getOrigin() as? KtClass ?: return
    ...
    try {
        ...
        val factoryFunction = when {
            currentBean.factoryBean.exists() -> {
                ...
                val ktFactoryClass = factoryClass.getOrigin() as? KtClass ?: return
            ...
            currentBean.factoryMethod.exists() -> {
                ...
                if (ktBeanClass.hasPrimaryConstructor()) {
        ...
        for ((i, psiParam) in dummyMethod.parameterList.parameters.withIndex()) {
            ...
            val ktParameter = psiFactory.createParameter(
                    "${psiParam.name}: ${IdeDescriptorRenderers.SOURCE_CODE.renderType(descriptor.type)}"
            )
...
private fun createConstructorDependency(
    ...
    currentBean.addConstructorArg().refAttr.stringValue = getReferencedName(currentBean, dependency)
...
fun generateDependenciesFor(
        ...
        klass: KtLightClass,
        injectionKind: SpringDependencyInjectionKind
): List<SpringGenerateTemplatesHolder> {
