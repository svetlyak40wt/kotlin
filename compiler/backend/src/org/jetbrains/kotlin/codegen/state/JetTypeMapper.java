...
import org.jetbrains.kotlin.codegen.signature.BothGenericSignatureWriter;
...
public class JetTypeMapper {
    ...
    public JvmMethodSignature mapAnnotationParameterSignature(@NotNull PropertyDescriptor descriptor) {
        BothSignatureWriter sw = new BothGenericSignatureWriter(BothGenericSignatureWriter.Mode.METHOD);
        sw.writeReturnType();
        ...
        if (functionParent instanceof ClassDescriptor) {
            ...
            if (isInterface && (superCall || descriptor.getVisibility() == Visibilities.PRIVATE || isAccessor(descriptor))) {
                ...
                if (declarationOwner instanceof JavaClassDescriptor) {
                    ...
                    signature = mapSignature(descriptor.getOriginal(), OwnerKind.DEFAULT_IMPLS, true);
    ...
    @NotNull
    public JvmMethodSignature mapSignature(@NotNull FunctionDescriptor f, boolean skipGenericSignature) {
    ...
    @NotNull
    public JvmMethodSignature mapSignature(@NotNull FunctionDescriptor f, @NotNull OwnerKind kind, boolean skipGenericSignature) {
    ...
    public JvmMethodSignature mapSignature(
        ...
        BothSignatureWriter sw = skipGenericSignature || f instanceof AccessorForCallableDescriptor
                                 ? new BothSignatureWriter()
                                 : new BothGenericSignatureWriter(BothGenericSignatureWriter.Mode.METHOD);
    ...
    public String mapFieldSignature(@NotNull KotlinType backingFieldType, @NotNull PropertyDescriptor propertyDescriptor) {
        BothSignatureWriter sw = new BothGenericSignatureWriter(BothGenericSignatureWriter.Mode.TYPE);
