...
public class DataFlowAnalyzer {
    ...
    private static FunctionDescriptor getFunctionDescriptorFromBaseClass(@NotNull FunctionDescriptor descriptor) {
        ...
        for (FunctionDescriptor overridden : overriddenDescriptors) {
            ...
            if (DescriptorUtils.isClass(containingDeclaration) || DescriptorUtils.isObject(containingDeclaration)) {
                ...
                return getFunctionDescriptorFromBaseClass(overridden);
            }
        }
        return getFunctionDescriptorFromBaseClass(overriddenDescriptors.iterator().next());
    }
    ...
    private static boolean typeHasOverriddenEquals(@NotNull KotlinType type, @NotNull KtElement lookupElement) {
        ...
        for (FunctionDescriptor member : members) {
            FunctionDescriptor fromBaseClass = getFunctionDescriptorFromBaseClass(member);
            ClassifierDescriptor superClassDescriptor = (ClassifierDescriptor) fromBaseClass.getContainingDeclaration();
